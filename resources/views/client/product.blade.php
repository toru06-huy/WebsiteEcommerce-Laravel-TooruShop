...
@push('styles')
<style>
.detail-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 60px; padding: 48px 0 80px; }
@media(max-width:900px){ .detail-layout { grid-template-columns: 1fr; gap: 32px; } }

/* ── LEFT: Images ── */
.img-gallery { display: flex; flex-direction: column; gap: 0; }
.img-main { position: relative; aspect-ratio: 3/4; overflow: hidden; background: var(--cream); margin-bottom: 10px; }
.img-main img { width: 100%; height: 100%; object-fit: cover; transition: transform .6s ease; }
.img-main:hover img { transform: scale(1.05); }
.img-thumbnails { display: flex; gap: 8px; flex-wrap: wrap; }
.img-thumb {
  width: 72px; height: 88px; overflow: hidden; cursor: pointer;
  border: 2px solid transparent; transition: var(--transition); flex-shrink: 0;
}
.img-thumb.active { border-color: var(--black); }
.img-thumb img { width: 100%; height: 100%; object-fit: cover; }

/* ── RIGHT: Info ── */
.product-detail-info { position: sticky; top: 96px; align-self: start; }
.detail-category { font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: var(--gold); margin-bottom: 12px; }
.detail-name { font-family: var(--font-display); font-size: clamp(24px,3vw,38px); font-weight: 300; line-height: 1.2; margin-bottom: 16px; }
.detail-price { font-size: 22px; color: var(--gold); font-weight: 500; margin-bottom: 28px; }

.option-label { font-size: 11px; letter-spacing: 1.5px; text-transform: uppercase; color: var(--black); margin-bottom: 10px; font-weight: 500; }
.option-label span { color: var(--gold); font-style: italic; letter-spacing: 0; text-transform: none; font-size: 13px; font-weight: 400; }
...
.color-swatch.active { border-color: var(--black); box-shadow: 0 0 0 3px var(--white), 0 0 0 5px var(--black); }
...
.color-swatch-text.active { border-color: var(--black); background: var(--cream); }
.color-swatch-text:hover { border-color: var(--black); }

/* Size options */
.size-options { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 28px; }
.size-btn {
  min-width: 52px; height: 52px; padding: 0 12px; border: 1px solid var(--border);
  font-size: 13px; cursor: pointer; transition: var(--transition); position: relative;
...
.size-btn.disabled::after { content:''; position:absolute;inset:0;display:flex;align-items:center;justify-content:center; }
...
.qty-ctrl button { width: 40px; height: 48px; font-size: 18px; transition: var(--transition); background: none; }
.qty-ctrl button:hover { background: var(--cream); }
.qty-ctrl input { width: 52px; height: 48px; text-align: center; border: none; border-left: 1px solid var(--border); border-right: 1px solid var(--border); font-size: 14px; outline: none; }

.btn-add-cart { flex: 1; height: 52px; }
.out-msg { padding: 12px; background: var(--cream); color: var(--gray); font-size: 13px; text-align: center; letter-spacing: .5px; }

/* Description */
.detail-divider { border: none; border-top: 1px solid var(--border); margin: 28px 0; }
.detail-desc { font-size: 14px; color: var(--gray); line-height: 1.9; }

/* Related */
.related-section { padding: 60px 0; border-top: 1px solid var(--border); }
.related-title { font-family: var(--font-display); font-size: 28px; font-weight: 300; margin-bottom: 28px; }
.related-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 20px; }
@media(max-width:768px){ .related-grid { grid-template-columns: repeat(2,1fr); } }
.related-card { cursor: pointer; }
.related-card:hover .rimg { transform: scale(1.04); }
.related-img-wrap { overflow: hidden; aspect-ratio: 3/4; background: var(--cream); }
.rimg { width: 100%; height: 100%; object-fit: cover; transition: transform .5s ease; }
