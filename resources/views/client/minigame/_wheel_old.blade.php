...
<div class="mg-result" id="mgResult">
  <div class="mg-result-code" id="mgResultCode"></div>
  <div class="mg-result-label" id="mgResultLabel"></div>
  <button type="button" class="mg-claim-btn" id="mgClaimBtn">Lấy mã</button>
...
(function () {
  ...
  const pendingResult = @json($pendingResult ?? null);
  ...
  function initWheelBlock(root) {
    ...
    const spinBtn   = root.querySelector('.mg-spin-btn');
    ...
    // Nếu có kết quả pending (chưa claim) từ lần quay trước, hiển thị luôn kết quả đó
    // if (pendingResult) {
    //   const targetCenter = pendingResult.index * segAngle + segAngle / 2;
    //   currentRotation = 5 * 360 - targetCenter;
    ...
    //   resultCode.textContent = pendingResult.code || '';
